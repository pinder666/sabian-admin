// populate_coords.cjs
// One-off script to populate lat/lon centroids in countries_canonical from iso2.
// Run: node populate_coords.cjs
...
// ISO-3166-1 alpha-2 → country centroid (lat, lon)
const CENTROIDS = {
  ...
  AX: { lat: 60.178524, lon: 19.915617 },
  ...
  MK: { lat: 41.512312, lon: 21.446633 },
  ...
  SS: { lat: 6.876991, lon: 31.306978 },
  ST: { lat: 0.18636, lon: 6.613081 },
  SV: { lat: 13.794185, lon: -88.89653 },
  SX: { lat: 18.034667, lon: -63.068333 },
  ...
  UM: { lat: 19.295374, lon: 166.6280441 },
...
async function main() {
  console.log('Fetching countries_canonical...');

  const { data: rows, error } = await sb
    .from('countries_canonical')
    .select('iso2, canonical_name');
  ...
  for (const row of rows) {
    ...
    const centroid = iso2 ? CENTROIDS[iso2] : null;

    if (!centroid) {
      noMatch++;
      missing.push(`${row.canonical_name} (${row.iso2 || 'no iso2'})`);
      continue;
    }

    const { error: upErr } = await sb
      .from('countries_canonical')
      .update({ lat: centroid.lat, lon: centroid.lon })
      .eq('iso2', row.iso2);

    if (upErr) {
      console.error(`Failed to update ${row.canonical_name}:`, upErr.message);
    } else {
      updated++;
    }
  }
  ...
  console.log(`No centroid match: ${noMatch}`);
  if (missing.length > 0) {
    console.log(`Missing iso2 matches:\n  ${missing.join('\n  ')}`);
  }
}
