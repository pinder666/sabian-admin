...
function run() {
  ...
  const sleepRecords = fitbit?.sleep?.summary?.totalSleepRecords ?? 0;
  ...
  const validSleep = sleepRecords > 0 && sleep > 0;
  const validSteps = steps > 0;

  const day = state.calibration_day || 1;

  console.log("CALIBRATION INPUT");
  console.log({ sleep, sleepRecords, steps, activeMinutes, validSleep, validSteps });
